...
import { exportToCSV, clearVisitors, deleteVisitor, fetchVisitorsFromServer, Visitor } from "@/lib/visitorStore";
import { getCases, hydrateCasesFromServer } from "@/lib/caseStore";
...
export default function VisitorsPage() {
    ...
    const loadData = async () => {
        ...
        const list = await fetchVisitorsFromServer();
        setVisitors(prev => {
            if (prev.length !== list.length) return list;
            const changed = list.some((item, i) => item.id !== prev[i]?.id);
            return changed ? list : prev;
        });
        setCaseVersion(getCases().length);
    ...
    const caseMap = useMemo(() => {
        const map = new Map<string, ReturnType<typeof getCaseByRelatedVisitorId>>();
        for (const visitor of visitors) {
            map.set(visitor.id, getCaseByRelatedVisitorId(visitor.id));
        }
        return map;
    }, [visitors, caseVersion]);
    ...
    const handleDeleteSingleVisitor = async (id: string, name: string) => {
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50 text-left border-b border-slate-100">
                                ...
                                <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Status Kunjungan</th>
                            ...
                            {isLoading ? (
                                ...
                                paginatedVisitors.map((v, idx) => (
                                    <tr key={v.id} className="hover:bg-slate-50/50">
                                        ...
                                        <td className="px-4 py-3 text-sm"><span className="inline-block px-2 py-0.5 bg-slate-100 text-slate-600 text-xs font-bold rounded-lg border border-slate-200">{getVisitorStatusLabel(v.status)}</span></td>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-end gap-2">
                                                {(() => {
                                                    ...
                                                    if (!c) return null;
                                                    return (
                                                        <Link href={`/admin/cases/${c.id}`} className="inline-flex items-center justify-center px-3 py-2 text-xs font-bold text-white bg-[#009FA9] rounded-xl hover:shadow-lg hover:-translate-y-0.5 transition-all shadow-sm whitespace-nowrap">
                                                            Kelola
                                                        </Link>
                                                    );
                                                })()}
                                                <button onClick={() => handleDeleteSingleVisitor(v.id, v.name)} className="text-red-500 hover:text-white hover:bg-red-500 p-2 rounded-xl transition-all shadow-sm border border-red-200 hover:border-red-500" title="Hapus Pengunjung">
                                                    {Icons.trash}
                                                </button>
                                            </div>
                                        </td>
