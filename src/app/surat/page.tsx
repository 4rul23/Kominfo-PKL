...
const STEP_CONFIG = [
    ...
    { title: "Jenis", num: 3 },
    { title: "Isi Surat", num: 4 },
    { title: "Lampiran", num: 5 },
    { title: "Konfirmasi", num: 6 },
];
...
export default function SuratPage() {
    ...
    useEffect(() => {
        setTimeout(() => {
            ...
            else if (step !== 3 && step !== 5) inputRef.current?.focus();
        }, 400);
    }, [step]);
    ...
    const handleNext = () => {
        ...
        if (step === 4 && (!data.perihal.trim() || !data.isiSurat.trim())) return;
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.key === "Enter" && step < TOTAL_STEPS && step !== 4 && step !== 5) handleNext();
    };
    ...
    return (
        <>
            {/* Hidden Receipt for Print - Rendered at top level to avoid inheritance issues */}
            {submittedSurat && <DigitalReceipt surat={submittedSurat} />}

            {/* Main Application Wrapper - Hidden during print */}
            <div className="fixed inset-0 z-50 flex flex-col items-center transition-all duration-500 ease-out overflow-y-auto scrollbar-hide pb-12 bg-[#f8fafc] print:hidden">
                ...
                <div className="absolute inset-0 z-0 overflow-hidden opacity-30 pointer-events-none">
                    ...
                    <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-[#991b1b]/5 rounded-full blur-[100px]" />
                </div>
                ...
                <header className="absolute top-0 left-0 w-full p-8 flex justify-between items-center z-20">
                    ...
                    <button onClick={handleClose} className="group flex items-center gap-2 px-4 py-2 rounded-full bg-white/50 border border-white/60 hover:bg-white hover:shadow-md transition-all text-[#505F79] hover:text-[#991b1b]">
                        ...
                        {step === TOTAL_STEPS + 1 && submittedSurat && (
                            <div className="text-center space-y-8 animate-fade-in-up">
                                ...
                                <p className="text-gray-400 text-sm max-w-md mx-auto">
                                    Simpan <strong className="text-[#172B4D]">Tracking ID</strong> untuk melacak status surat Anda.
                                </p>

                                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                    <button onClick={() => window.print()} className="px-8 py-4 bg-white border-2 border-gray-200 text-[#505F79] font-bold rounded-2xl hover:border-[#009FA9] hover:text-[#009FA9] transition-all flex items-center gap-2 justify-center">
